...
public class QRCodeGenerator {
    ...
    public String generateQRUrl(UserAccountEntity userAccountEntity) throws UnsupportedEncodingException {
        return QR_PREFIX + URLEncoder.encode(String.format(
                "otpauth://totp/%s:%s?secret=%s&issuer=%s",
                APP_NAME, userAccountEntity.getUsername(), userAccountEntity.getSecret(), APP_NAME), "UTF-8");
    }
}
